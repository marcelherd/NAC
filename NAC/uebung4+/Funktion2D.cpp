...
Vektor2D const& minimieren2D(const Funktion2D &f, const Vektor2D &start, float step, int maxIterations, float tol, float ftol)
{
	...
	Vektor2D gradientNorm;
	gradient.kopiereIn(&gradientNorm);
	if (gradient.betrag() != 1) {
		gradientNorm = (gradientNorm * (1 / gradientNorm.betrag()));
	}

	float richtung = gradient.skalarprodukt(gradientNorm);
	gradient.ausgabe();
